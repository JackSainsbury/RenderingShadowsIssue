...
int main() {
    ...
    from = glm::vec3(0.0f,0.0f,6.0f);
    to = glm::vec3(0.0f,-0.5f,0.0f);

    // Needed for the fixed camera
    g_camera.setTarget(to.x,to.y,to.z);
    g_camera.setEye(from.x,from.y,from.z);

    g_scene.setCameraPos(from);
    g_scene.setLightPos(glm::vec3(-4.0, 6.0f, -4.0f));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        auto t = std::chrono::duration_cast<std::chrono::milliseconds>(start - std::chrono::high_resolution_clock::now());

        g_scene.setLightPos(glm::vec3(4 * sin (t.count()/1000.0f),2,4 * cos(t.count()/1000.0f)));
