#version 430
uniform sampler2D ColourTexture1;
...
void main() {
    ...
    vec3 tgt = normalize(texture(NormalTexture1, vec2(WSTexCoord.x+0.13,WSTexCoord.y-0.03)*4).rgb * 2.0 - 1.0);
   // vec3 tgt = normalize(texture(NormalTexture, vec2(WSTexCoord.x+0.13,WSTexCoord.y-0.03)*4).rgb * 2.0 - 1.0);
    ...
    vec3 texColor = texture(ColourTexture1, vec2(WSTexCoord.x+0.13,WSTexCoord.y-0.03)*4).rgb;

    // Set the output color of our current pixel
    FragColor = vec4((texColor-vec3(0.3,0.3,0.3))*lightColor, 1.0);
